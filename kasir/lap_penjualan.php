...
if(isset($_POST['button_filter'])){
	...
	$query_get_faktur="SELECT tabel_penjualan.*,tabel_rinci_penjualan.no_faktur_penjualan FROM tabel_penjualan,tabel_rinci_penjualan WHERE tabel_penjualan.no_faktur_penjualan=tabel_rinci_penjulan.no_faktur_penjualan AND tabel_penjualan.tgl_penjualan BETWEEN '".$tanggal_awal."' AND '".$tanggal_akhir."' AND tabel_penjualan.no_faktur_penjualan LIKE'".$kd_toko."%' GROUP BY tabel_penjualan.no_faktur_penjualan";}
	else{
	$tanggal=false;
	$kd_toko=$_SESSION['kd_toko'];
	$query_get_faktur="SELECT tabel_penjualan.*,tabel_rinci_penjualan.no_faktur_penjualan FROM tabel_penjualan,tabel_rinci_penjualan WHERE tabel_penjualan.no_faktur_penjualan=tabel_rinci_penjulan.no_faktur_penjualan AND tabel_penjualan.no_faktur_penjualan LIKE'".$kd_toko."%' GROUP BY tabel_penjualan.no_faktur_penjualan";}
	$get_faktur=mysql_query($query_get_faktur);
	if(!$get_faktur) {echo "bisa kok";}
...
if($_GET['view']=="cabang"&&$_SESSION['status_toko']=="pusat"){
	...
	while($get_kd_toko=mysql_fetch_array($query_get_kd_toko)){
        ...
        <input type="submit" name="button_print" id="button_print" value="print" onclick="print()"/></td>td>
        </tr>
        </table>
        </form></div>
        ...
        <p>Tanggal:<?php if($tanggal==true){echo $tanggal_awal_format."s/d".$tanggal_akhir_format;}?>
		</p></td>
        </tr>
        <?php
